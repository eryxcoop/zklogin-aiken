...
* Create a ```.env``` file in the ```backend``` directory based on the ```.env.example``` and set your own Blockfrost key for the network you want to use (preview or preprod).
...
Note: all of this has been tested on the preprod network. The preview network has recently gone for some protocol changes that break most of the Cardano libraries to deploy transactions. Users currently have a dropdown at the top of the webpage to choose either `preview` or `preprod` (default is `preprod`). This dropdown is only gonna be used for steps 7 and 8 in the demo, and users don't have to restart the demo when it changes from one to another.
